...
int main(int argc, char* argv[]){
  ...
  double compute_start, compute_end, compute_time = 0, compute_count = 0, compute_total = 0;
  double comm_start, comm_end, comm_time = 0, comm_count = 0, comm_total = 0;
  ...
  for(i = 0; i < iter; i++){
    ...
    if(rank == 0){
    	/* does not performa ghost row exchange if only 1 process exists */
    	if(size > 1){
    ...
    if(rank == 0){
    	/* rank 0 adds its own results first */
	...
	int o;
	for(o = 1; o < size; o++){
	  MPI_Recv(&tmp, 1, MPI_DOUBLE, o, 2, MPI_COMM_WORLD, &status);
	  compute_total += tmp;
	  compute_count++;
	  
	  MPI_Recv(&tmp, 1, MPI_DOUBLE, o, 3, MPI_COMM_WORLD, &status);
	  comm_total += tmp;
	  comm_count++;	
	}
    }
    	...
    	MPI_Send(&comm_time, 1, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD);
    }
    MPI_Barrier(MPI_COMM_WORLD);
  }
  ...
  if(rank == 0){
    ...
    printf("Computation average time (compute_time): %0.10f\n", (compute_total / compute_count));
    printf("Communication average time (comm_time): %0.10f\n", (comm_total/ comm_count));
    printf("\n");
  }
...
/* prints the image in image file format */
void PrintGUI(float*** grid, int dime){
  int i, j;
  FILE *fp;
